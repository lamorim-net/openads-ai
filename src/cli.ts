...
function buildSystemPrompt(config: any): string {
  ...
  parts.push(
    ...
    '- **Core Loop (Triggered by "Launch the Autoresearch core loop to generate new testable marketing hypotheses.")**: Read `autoresearch.md` skill. You MUST NOT guess or automatically run loop cycles if a target Goal and Metric are not explicitly defined by the user in this session. If not explicitly specified in the conversation, halt immediately and ask the user to input their custom Goal and Metric before launching. Once defined, run at least 3 autonomous cycles (Generate ➡️ Score ➡️ Keep/Discard ➡️ Iterate), output cycle logs, final table of new hypotheses, and auto-save to `~/.openads/reports/`.',
